...
template< typename T >
using kd_tree = geometricks::leaf_kd_tree<T>;
...
TEST( TestLeafKDTree, TestNearestNeighborCustomFunction ) {
  std::vector<std::tuple<int, int, int>> input_vector;
  input_vector.reserve( 10 );
  input_vector.push_back( std::make_tuple( 50, 50, 50 ) );
  input_vector.push_back( std::make_tuple( 0, 49, 87 ) );
  ...
  custom_nearest_neghbor_function::calls_int_int = 0;
  custom_nearest_neghbor_function::calls_int_int_dim1 = 0;
  custom_nearest_neghbor_function::calls_int_int_dim2 = 0;
  custom_nearest_neghbor_function::calls_tuple_int_dim2 = 0;
  std::vector<std::pair<std::tuple<int, int, int>, size_t>> output_vector;
  output_vector.reserve( 1 );
  auto [nearestK, distanceK] = tree.k_nearest_neighbor( std::make_tuple( 10, 10, 10 ), 1, output_vector, custom_nearest_neghbor_function{} )[ 0 ];
  EXPECT_EQ( nearest, nearestK );
  EXPECT_EQ( distance, distanceK );
  EXPECT_EQ( nearest, std::make_tuple( 10, 9, 11 ) );
  EXPECT_EQ( custom_nearest_neghbor_function::calls_int_int, 1 );
  EXPECT_EQ( custom_nearest_neghbor_function::calls_int_int_dim1, 1 );
  EXPECT_EQ( custom_nearest_neghbor_function::calls_int_int_dim2, 0 );
