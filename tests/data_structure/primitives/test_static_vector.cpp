...
TEST( TestStaticVector, TestSafePopPopulatedVector ) {
  tcc::data_structure::safe_static_vector<int, 4> vec{};
...
TEST( TestStaticVector, TestDestructorCalls ) {
  tcc::data_structure::safe_static_vector<not_trivially_destructible, 2> outer;
  not_trivially_destructible copy_me;
  outer.push_back(copy_me, copy_me);
  EXPECT_EQ( not_trivially_destructible::m_calls, 0 );
  {
    tcc::data_structure::safe_static_vector<not_trivially_destructible, 42> inner;
    for( int i = 0; i < 38; ++i ) {
...
TEST( TestStaticVector, TestCopyConstructor ) {
  tcc::data_structure::safe_static_vector<int, 8> original;
  original.push_back( 1, 2, 3, 4 );
  tcc::data_structure::safe_static_vector<int, 8> copy{ original };
  for( int i = 0; i < 4; ++i ) {
...
TEST( TestStaticVector, TestCopyAssignment ) {
  tcc::data_structure::safe_static_vector<int, 8> original;
  original.push_back( 1, 2, 3, 4 );
  tcc::data_structure::safe_static_vector<int, 8> copy;
  copy = original;
  for( int i = 0; i < 4; ++i ) {
    EXPECT_EQ( original[ i ], i + 1 );
    EXPECT_EQ( original[ i ], copy[ i ] );
...
TEST( TestStaticVector, TestMoveConstructor ) {
  ...
  tcc::data_structure::safe_static_vector<int, 8> original;
  original.push_back( 1, 2, 3, 4 );
  tcc::data_structure::safe_static_vector<int, 8> copy{ std::move( original ) };
  for( int i = 0; i < 4; ++i ) {
    EXPECT_EQ( original[ i ], i + 1 );
    EXPECT_EQ( original[ i ], copy[ i ] );
    copy[ i ]++;
