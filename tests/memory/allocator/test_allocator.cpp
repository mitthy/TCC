...
TEST( TestAllocator, TestDefaultAllocator ) {
  ...
  EXPECT_EQ( default_allocator, geometricks::memory::malloc_allocator );
  ...
  *allocated = 42;
  ...
  EXPECT_FALSE( geometricks::memory::get_default_allocator() == geometricks::memory::malloc_allocator );
...
TEST( TestAllocator, TestDefaultConstructor ) {
  ...
  EXPECT_EQ( default_constructed_allocator, geometricks::memory::malloc_allocator );
  int* allocated = nullptr;
